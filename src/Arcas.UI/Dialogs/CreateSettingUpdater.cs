...
{
    private class ErrorTracker(ErrorProvider provider)
    {
        private HashSet<Control> mErrors = [];
        ...
        public override string ToString() =>
            !mErrors.Any()
                ? null
                : mErrors.Select(provider.GetError).JoinValuesToString(Environment.NewLine);
    }
    ...
    private void dbSettingLoad(UpdateDbSetting upsets)
    {
        ...
        if (upsets.TypeConnectionFullName != conn.ToString())
        {
            ...
            if (conn == null)
            {
                ...
                foreach (var la in upsets.LinkedAssemblyDbConnection ?? [])
    ...
    private void validateAll()
    {
        ...
        tbFormatVersion_Validating(null, null);
        tbNumberTask_Validating(null, null);
    }

    private void tbFormatVersion_Validating(object sender, System.ComponentModel.CancelEventArgs e)
    {
        if (tbFormatVersion.Text.IsNullOrWhiteSpace())
            errorTracker.SetError(tbFormatVersion, "Не указан шаблон формирования версии");
        else
            errorTracker.SetError(tbFormatVersion, null);
    }
