...
public partial class UpdaterDB : TabControlBase
{
    ...
    private FormatBinaryData formatbin;
    ...
    private void bttvQueryRefresh_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (cbxTfsDbLinc.SelectedItem is not TfsDbLink curset || curset.ServerUri == null || curset.ServerPathToSettings.IsNullOrWhiteSpace())
            {
    ...
    private void tvQuerys_AfterSelect(object sender, TreeViewEventArgs e)
    {
        try
        {
            ...
            if (cbxTfsDbLinc.SelectedItem is not TfsDbLink curset)
                return;
    ...
    {
        foreach (var item in lbWorkItems.SelectedItems.Cast<Lwi>())
        {
            var si = item;
