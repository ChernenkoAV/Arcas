...
#pragma warning restore CA1054 // Параметры, напоминающие URI, не должны быть строками
        ...
        string outputFile)
    {
        ...
        var sb = DynamicCodeHelper.GenerateCodeFromXsd(XDocument.Load(pd.MainTempFile), pd.TargetCSNamespace, pd.Imperts.Select(XDocument.Load).ToArray());
    ...
    private IEnumerable<String> downloadImport(string filePath, string tempDir, Uri sourseUri, String originLocationFile)
    {
        ...
        void lartImp(XAttribute locationAttrib, string fileintemp)
        {
